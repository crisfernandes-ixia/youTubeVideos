...
def main():
    ...
    # Adding QOS to VLAN priority Field
    ## Value Type Available - "singleValue","valueList","increment","decrement","random","repeatableRandomRange"
    vlanStackObj = config_elem_1.Stack.find(DisplayName='VLAN')
    vlanIdPriority = vlanStackObj.Field.find(DisplayName='VLAN Priority')
    vlanIdPriority.ValueType = 'valueList'
    vlanIdPriority.ValueList = [0,1,2,3]

    vlanStackObj = config_elem_2.Stack.find(DisplayName='VLAN')
    vlanIdPriority = vlanStackObj.Field.find(DisplayName='VLAN Priority')
    vlanIdPriority.ValueType = 'singleValue'
    vlanIdPriority.SingleValue = 4

    vlanStackObj = config_elem_3.Stack.find(DisplayName='VLAN')
    vlanIdPriority = vlanStackObj.Field.find(DisplayName='VLAN Priority')
    vlanIdPriority.ValueType = 'increment'
    vlanIdPriority.StartValue = 5
    vlanIdPriority.StepValue = 0

    vlanStackObj = config_elem_4.Stack.find(DisplayName='VLAN')
    vlanIdPriority = vlanStackObj.Field.find(DisplayName='VLAN Priority')
    vlanIdPriority.ValueType = 'repeatableRandomRange'
    vlanIdPriority.CountValue = 2
    vlanIdPriority.MaxValue = 7
    vlanIdPriority.MinValue = 6
    vlanIdPriority.Seed = 12345

    traff_item_server_to_clients.Tracking.find()[0].TrackBy = ["trackingenabled0", "flowGroup0","vlanVlanUserPriority0"]
    ...
    for queueId, stats in resultsDict.items():
        ...
        ixnet_session.info(f"Vlan Priority:{queueId} TX Frames:{txFrames:,} RX Frames:{rxFrames:,}")
        if txFrames == 0:
            ixnet_session.info(f"Vlan Priority:{queueId} - No traffic sent, skipping comparison")
            continue
        if compare_numbers(txFrames,rxFrames):
            ixnet_session.info(f"Vlan Priority:{queueId} - Test PASS")
        else:
            ixnet_session.info(f"Vlan Priority:{queueId} - Test FAILED")            
